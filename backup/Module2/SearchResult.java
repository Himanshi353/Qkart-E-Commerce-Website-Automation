    ...
    public Boolean verifySizeChartExists() {
        ...
        try {
            ...
            if(sizechartbtn.isDisplayed()){
                String text=sizechartbtn.getText();
        ...
        try {
            ...
            List<String> rowData=expectedTableBody.get(i);
             for(int j=0;j<rowData.size();j++){
                String expectedTBodyValue=rowData.get(j);
                int row=i+1;
                int column=j+1;
                WebElement tableContents=driver.findElement(By.xpath("//table/tbody/tr["+row+"]/td["+column+"]"));
                String actualTBodyValue=tableContents.getText();
                if(!expectedTBodyValue.equals(actualTBodyValue)){
                    status=false;
                }
             }
            }
